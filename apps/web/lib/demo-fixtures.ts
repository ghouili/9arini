...
/* The demo fixtures themselves — fictitious tutors, classes and a pack.
   ...
   IMPORT ONLY FROM lib/demo.ts, which gates every read behind `demoEnabled`.

   NOT IN ANY PRODUCTION BUILD. next.config.mjs swaps this module for
   demo-fixtures.empty.ts whenever webpack builds for production. Gating the
   calls was not enough on its own: the 15 Sept production build still shipped
   these bodies — "Yassine Khelifi", his classes, "Sonia Trabelsi" — in the
   /explore client chunk and in the server chunks, because the minifier does not
   remove functions behind an exported constant. Inert, but readable by anyone
   who opens the bundle, and the header of lib/demo.ts promised otherwise.

   No rating, no student count, no free-session promise in a bio: see lib/demo.ts. */
...
export const devStorefronts = (): Record<string, Storefront> => ({
